...
type WireDrawingProduct = {
  ...
  saleOrderLine?: string | null;
  fgProduct?: string | null;
  packingLength?: number | string | null;
  inletSize?: number | string | null;
  finishSize?: number | string | null;
  packingStandards?: string | null;
  totalSpoolQty?: number | string | null;
  customerInspection?: string | null;
  ppdDate?: string | null;
  wipPlanQty?: number | string | null;
...
type WireDrawingApiJob = {
  ...
  products?: WireDrawingProduct[];
};

type WireDrawingCard = {
  title: string;
  processLabel: string;
  jobName: string;
  jobNumber: string;
...
const dummyWireDrawingProductInfo = {
  inletSize: "9.50 mm",
  finishSize: "2.04 mm",
  packingStandards: "630mm (H/Blue)",
  totalSpoolQty: "1 x 36 x 1444.6698",
};
...
function extractSize(description?: string | null, productCode?: string | null) {
  const descriptionText = safeText(description, "");
  const diaMatch = descriptionText.match(/(?:dia|diameter)\s*([0-9.]+\s*mm)/i);
  if (diaMatch?.[1]) return diaMatch[1];

  const mmMatch = descriptionText.match(/([0-9.]+\s*mm(?:²|2)?)/i);
  if (mmMatch?.[1]) return mmMatch[1];

  const codeText = safeText(productCode, "");
  const codeMatch = codeText.match(/(\d{2,4}\.\d{2,4})/);
  if (codeMatch?.[1]) return codeMatch[1];

  return "-";
}

function mapWireDrawingJob(data?: WireDrawingApiJob | any): WireDrawingCard {
  const product: WireDrawingProduct = data?.products?.[0] ?? {};
  const plannedQty = toNumber(product.plannedQty ?? product.wipPlanQty ?? product.originalQty, 0);
  const actualQty = toNumber(product.executedQty ?? product.qty, 0);
  const remainingQty = toNumber(product.remainingQty ?? product.allocatedQty ?? Math.max(plannedQty - actualQty, 0), 0);
  const orderLength = toNumber(product.originalQty ?? product.wipPlanQty ?? plannedQty, 0);
  const length = toNumber(product.packingLength ?? product.balance ?? actualQty, 0);
  const processLabel = titleCase(safeText(data?.process ?? data?.processName ?? product.process, "Drawing"));

  return {
    title: processLabel,
    processLabel,
    jobName: safeText(data?.jobName ?? data?.jobNumber, "WIRE DRAWING"),
    jobNumber: safeText(data?.jobNumber ?? data?.jobName),
    workOrderNo: safeText(product.workOrder ?? product.workOrders?.join(", ")),
    machine: safeText(data?.machine),
    operation: safeText(data?.processName ?? product.process ?? data?.process),
    size: extractSize(product.description, data?.productCode ?? product.product),
    orderLength,
    cSpec: safeText(product.description ?? product.product ?? data?.productCode),
    inletSize: safeText(product.inletSize, dummyWireDrawingProductInfo.inletSize),
    finishSize: safeText(product.finishSize, dummyWireDrawingProductInfo.finishSize),
    packingStandards: safeText(product.packingStandards, dummyWireDrawingProductInfo.packingStandards),
    totalSpoolQty: safeText(product.totalSpoolQty, dummyWireDrawingProductInfo.totalSpoolQty),
    batchNo: safeText(product.batchNo),
    plannedHours: safeText((data as any)?.plannedHours ?? (product as any)?.plannedHours),
    actualQty,
    plannedQty,
    remainingQty,
    length,
    customerName: safeText(product.customerName),
    productCode: safeText(data?.productCode ?? product.product),
    description: safeText(product.description),
    saleOrderLine: safeText(product.saleOrderLine),
    fgProduct: safeText(product.fgProduct),
    ppdDate: formatDate(product.ppdDate),
    status: safeText(data?.status, "Ready"),
  };
}
  ...
  return (
    <div className="rounded-xl bg-white/10 px-2 py-1.5">
      <div className="flex items-center justify-between gap-2">
        <span className="truncate text-[9px] font-bold text-white/70">{label}</span>
        <span className="truncate text-[13px] font-black">{value}</span>
      </div>
    </div>
  );
}
...
export default function WireDrawing({ onBack, data, onLogout }: Props) {
  ...
  const orderLength = jobCard.orderLength || jobCard.plannedQty || 1;
  const displayProducedLength = producedLength || jobCard.actualQty || jobCard.length || 0;
  ...
  const statusText = useMemo(() => {
    switch (status) {
      ...
      case "COMPLETED":
        return "COMPLETED";
      default:
        return jobCard.status.toUpperCase() === "ACTIVE" ? "READY" : "READY";
    }
  ...
  const finalize = () => {
    ...
    if (workflow === "changeDrum") {
      ...
      if (nextInputDrum) setInputDrumValue(nextInputDrum);
      if (nextOutputDrum) setOutputDrumValue(nextOutputDrum);
      pushEvent(`Spool changed: ${nextInputDrum || inputDrumValue || "-"} -> ${nextOutputDrum || outputDrumValue || "-"} | ${lengthInput || displayProducedLength} m`);
      setStatus("RUNNING");
    } else if (workflow === "qcHold") {
      setStatus("QC_HOLD");
      pushEvent(`QC Hold committed at ${lengthInput || displayProducedLength} m: ${reason}${remarks ? ` | ${remarks}` : ""}`);
    } else if (workflow === "stop") {
      setStatus("STOPPED");
      pushEvent(`Stop job committed: ${reason}${remarks ? ` | ${remarks}` : ""}`);
    } else if (workflow === "fault") {
      setStatus("FAULT");
      pushEvent(`Fault logged at ${lengthInput || displayProducedLength} m: ${reason}${remarks ? ` | ${remarks}` : ""}`);
    } else if (workflow === "materialIssue") {
      setStatus("MATERIAL_ISSUE");
      pushEvent(`Material issue logged at ${lengthInput || displayProducedLength} m: ${reason}${remarks ? ` | ${remarks}` : ""}`);
      ...
      pushEvent(`Rewind committed at ${lengthInput || displayProducedLength} m | next: ${nextAfterRewind}`);
      ...
      setStatus("COMPLETED");
      pushEvent(`Job completed at ${lengthInput || displayProducedLength} m${remarks ? ` | ${remarks}` : ""}`);
    }
      ...
      <div className="mx-auto flex max-w-[1400px] flex-col gap-2">
        <header className="sticky top-1 z-30 rounded-2xl border border-slate-200 bg-white/95 px-2 py-1.5 shadow-sm backdrop-blur">
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
            ...
            <div className="rounded-full bg-slate-100 px-3 py-1 text-[13px] font-black tracking-tight">{jobCard.processLabel.toUpperCase()}</div>
            ...
            <div className="flex items-center justify-end gap-1.5">
              ...
              <button type="button" onClick={() => pushEvent("Refreshed")} className="grid h-8 w-8 place-items-center rounded-full border border-slate-200 bg-white">
              ...
              <div className="text-left">
                <div className="flex items-end justify-start gap-2">
                  ...
                  <div className="pb-1 text-[12px] font-black">
                    {formatQty(displayProducedLength)} m / {formatQty(orderLength)} m
                  </div>
                </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="border-t border-slate-200 p-2 pt-1.5">
            <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6">
              {jobInfoRows.map(([label, value]) => (
                <InfoBox key={label} label={label} value={value} />
              ))}
            </div>

            <div className="mt-2 grid grid-cols-2 gap-1.5 md:grid-cols-4">
              {/* <InfoBox label="Customer" value={jobCard.customerName} /> */}
              {/* <InfoBox label="Sale Order Line" value={jobCard.saleOrderLine} /> */}
              {/* <InfoBox label="FG Product" value={jobCard.fgProduct} /> */}
              {/* <InfoBox label="Product Code" value={jobCard.productCode} /> */}
            </div>
          </div>
        </section>
        ...
        {showInstructions && (
          <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm font-semibold text-slate-700">
              No instructions configured yet for {jobCard.processLabel}.
            </div>
          </section>
        )}
        ...
        <footer className="sticky bottom-1 z-20 rounded-2xl border border-slate-200 bg-white px-3 py-2 shadow-sm">
          <div className="grid grid-cols-3 items-center gap-2 text-[11px] font-bold text-slate-700">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <button type="button" onClick={() => pushEvent("Refresh clicked")} className="grid h-7 w-7 place-items-center rounded-full border border-slate-200">
      ...
      {workflow && (
        ...
        >
          <div className="grid gap-4">
              ...
              <div className="grid gap-2 md:grid-cols-4">
                {[
                  ...
                  <button
                    key={item.key}
                    type="button"
                    onClick={() => setDecisionChoice(item.key as any)}
            ...
            <button type="button" onClick={workflow === "startJob" ? commitStartJob : finalize} className="rounded-2xl bg-green-700 py-3 font-black text-white">
              Commit / محفوظ کریں
            </button>
          </div>
        </Modal>
