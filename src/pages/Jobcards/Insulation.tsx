...
export default function Insulation({ onBack, data, onLogout }: Props) {
  ...
  <div className="grid grid-cols-4 gap-2">
    <div className="rounded-xl border border-slate-200 bg-white px-2 py-2 shadow-sm">
      ...
      <div className="truncate text-[12px] font-black leading-tight">{DEFAULT_MACHINE}</div>
      <div className="truncate text-[10px] font-semibold leading-tight text-slate-500">{DEFAULT_MACHINE_SUB}</div>
    </div>
    <div className="rounded-xl border border-slate-200 bg-white px-2 py-2 shadow-sm">
    ...
    <div className="rounded-xl border border-slate-200 bg-white px-2 py-2 shadow-sm">
      ...
      <div className="truncate text-[10px] font-semibold leading-tight text-slate-500">Approved</div>
          ...
          <div className="border-t border-slate-200 p-2 pt-1.5">
            <div className="grid grid-cols-4 gap-1.5 md:grid-cols-8">
              {[
                ...
                ["Color", data?.color || "1"],
                ["Insulation thickness", data?.thickness || "0.8 mm"],
                ["Core size", data?.coreSize || "N/A"],
                ["OD", data?.od || "24.25 mm"],
                ["Spark test", data?.sparkTest || "OK"],
                ["Line speed", data?.lineSpeed || "120 m/min"]
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
          <LayingUpActionPanel cards={STANDARD_ACTION_CARDS} onAction={handleAction} disabledKeys={disabledKeys} />
        </section>
        ...
        {showInstructions && (
          <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm font-semibold text-slate-700">
              No instructions configured yet for Insulation.
            </div>
          </section>
        )}
